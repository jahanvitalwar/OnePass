...
root.iconbitmap("Images/jahanvilogo.ico")
width = 500
height = 300
...
def signup():
    if (username_entry.get() == "" or password_entry.get() == ""):
        ...
        try:
            main_def_func.tableCreate()  # Searches to see if database has already been created
...
def info():
    messagebox.showinfo("About", "This is a password manager where you can safely store all your passwords without worrying to forget them. You only need to remember ONE password to login and retrieve all your stored passwords for various accounts.")
